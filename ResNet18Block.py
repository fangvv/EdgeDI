...
class BasicBlock(nn.Module):
    def __init__(self, inchannel, outchannel, strid):
        ...
        temp_outchannel = int(outchannel/8)
        self.conv1 = nn.Sequential(
            nn.Conv2d(inchannel, temp_outchannel, kernel_size=1, padding=0, bias=False),
            nn.Conv2d(temp_outchannel, temp_outchannel, kernel_size=3, stride=strid, padding=0, bias=False),
            Feature_enhancement(temp_outchannel),
            nn.Conv2d(temp_outchannel, outchannel, kernel_size=1, padding=0, bias=False),
        )

        self.conv2 = nn.Sequential(
            nn.Conv2d(outchannel, temp_outchannel, kernel_size=1, padding=0, bias=False),
            nn.Conv2d(temp_outchannel, temp_outchannel, kernel_size=3, stride=1, padding=0, bias=False),
            Feature_enhancement(temp_outchannel),
            nn.Conv2d(temp_outchannel, outchannel, kernel_size=1, padding=0, bias=False),
...
class resnet18Blockx(nn.Module):
    def __init__(self, layer_num=None):
        super(resnet18Blockx, self).__init__()
